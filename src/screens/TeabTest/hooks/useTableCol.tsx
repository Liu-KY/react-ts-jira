...
const useTableCol = (
  ...
  const titleWidthMapRef = useRef<{
    titleWidthMap: Record<string, number> | undefined;
  }>({ titleWidthMap: undefined });
  // 每一列的宽度转换成数字之后的列配置
  ...
  const getTableNumberWidthCol = useMemoizedFn(() => {
    ...
    if (wrapperWidth && columns) {
      ...
      if (noWidthColumes.length > 0) {
        ...
        if (otherWidth > 0) {
          ...
          const resultColumes = newColumesList.map((col) => {
            if (!col.width) {
              // 最后一个未配置宽度的列宽取差值
              if (
                col.title === noWidthColumes[noWidthColumes.length - 1].title
              ) {
                col.width =
                  otherWidth - commonWidth * (noWidthColumes.length - 1);
              } else {
          ...
          const showColWithWidth =
            newColumesList.length - noWidthColumes.length;
        ...
        const lastWidth =
          otherWidth - averageWidth * (newColumesList.length - 1);
  ...
  const setTitleWidthMapMethod = useMemoizedFn(
    (colList: ColumnType<any>[], allWidth?: number) => {
      if (allWidth) {
        const result: Record<string, number> = {};
        colList.forEach(({ width }, index) => {
          result[`_${index}`] = parseFloat(
            ((width as number) / allWidth).toFixed(2)
          );
        });
    ...
    if (wrapperWidth && isInit) {
      setTableShowColumns((oldColumns) => {
        ...
        result[result.length - 1].width =
          wrapperWidth +
          parseFloat(`${result[result.length - 1].width!}`) -
          totalWidth;
        return result;
      });
    }
